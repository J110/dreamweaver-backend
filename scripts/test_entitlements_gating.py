...
def _native_flag_off():
    # native flag is a process-global contextvar monkeypatch won't restore;
    # reset around every test so state can't leak between tests.
...
def test_is_premium_unknown_tier_false(monkeypatch):
    _gate_on(monkeypatch)
    assert gating.is_premium({"subscription_tier": "pro"}) is False
