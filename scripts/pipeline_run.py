...
def step_generate(args, state: dict) -> bool:
    ...
    if actual < expected and not args.dry_run:
        ...
        for retry_attempt in range(1, 3):
            ...
            logger.info(
                "  RETRY %d/2: Attempting to generate %d missing items "
                "(stories=%d, long=%d, poems=%d, lullabies=%d)",
                retry_attempt, total_missing, missing_stories, missing_long,
                missing_poems, missing_lullabies,
            )
            ...
            if effective_mood:
                retry_cmd += ["--mood", effective_mood]
            if effective_story_type:
                retry_cmd += ["--story-type", effective_story_type]
            if args.age:
                retry_cmd += ["--age", args.age]

            retry_ok, _, _, _ = run_command(
                retry_cmd,
                f"Content Generation (retry {retry_attempt}/2)",
                timeout=1800,
            )
            if retry_ok:
                ...
                if retry_ids:
                    logger.info(
                        "  RETRY SUCCESS: Generated %d additional items",
                        len(retry_ids),
                    )
...
def _pick_before_bed_age() -> str:
    """Rotate age group daily: 2-5 → 6-8 → 9-12 based on day-of-year."""
    ages = ["2-5", "6-8", "9-12"]
...
def step_before_bed(args, state: dict) -> bool:
    ...
    silly_cmd = [
        sys.executable, str(SCRIPTS_DIR / "generate_silly_songs_battlecry.py"),
        "--fresh", "--count", "1", "--age", age,
    ]
    if mood:
        silly_cmd += ["--mood", mood]
    ok, stdout, stderr, elapsed = run_command(
    ...
    ok, stdout, stderr, elapsed = run_command(
        [sys.executable, str(SCRIPTS_DIR / "generate_experimental_poems.py"),
         "--age", age, "--mood", mood],
        "Before Bed: generate musical poem",
        timeout=300,
    )
    if ok:
        for line in (stdout or "").split("\n"):
            if line.strip().startswith("["):
                before_bed_results["poem"] = line.strip()
                break
...
def step_sync(args, state: dict) -> bool:
    ...
    if web_audio_dir.exists() and backend_audio_dir.exists():
        ...
        for cover_subdir in ["silly-songs", "poems", "funny-shorts", "funny-shorts-hi"]:
            ...
            web_sub.mkdir(parents=True, exist_ok=True)
            synced = 0
            for f in list(backend_sub.glob("*.svg")) + list(backend_sub.glob("*.webp")):
                dest = web_sub / f.name
                if not dest.exists() or f.stat().st_size != dest.stat().st_size:
                    shutil.copy2(f, dest)
                    synced += 1
            if synced:
                logger.info("  Synced %d %s cover files to web public", synced, cover_subdir)
