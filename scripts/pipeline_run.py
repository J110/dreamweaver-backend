...
def _count_todays_mood_content() -> dict:
    """Count content generated today by experimental mood runs.

    Scans content_expanded.json for items with a 'mood' field whose
    created_at is today (UTC). Returns counts by type so the main
    pipeline can reduce its generation targets accordingly.
    """
    counts = {"story": 0, "long_story": 0, "poem": 0, "song": 0}
    if not CONTENT_EXPANDED_PATH.exists():
        return counts
    try:
        items = json.loads(CONTENT_EXPANDED_PATH.read_text())
...
def step_generate(args, state: dict) -> bool:
    ...
    if not args.mood:
        mood_counts = _count_todays_mood_content()
        mood_total = sum(mood_counts.values())
        if mood_total > 0:
            logger.info("  Experimental mood content found today: %s", mood_counts)
            # Subtract mood-generated counts from daily targets
            count_stories = max(0, count_stories - mood_counts["story"])
            count_long_stories = max(0, count_long_stories - mood_counts["long_story"])
            count_poems = max(0, count_poems - mood_counts["poem"])
            count_lullabies = max(0, count_lullabies - mood_counts["song"])
            skipped = []
            if mood_counts["story"]: skipped.append(f"{mood_counts['story']} story")
            if mood_counts["long_story"]: skipped.append(f"{mood_counts['long_story']} long story")
            if mood_counts["poem"]: skipped.append(f"{mood_counts['poem']} poem")
            if mood_counts["song"]: skipped.append(f"{mood_counts['song']} lullaby")
            logger.info("  Skipping %s (already generated by experimental run)", ", ".join(skipped))
            state["mood_skip"] = mood_counts
