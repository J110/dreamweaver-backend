...
def generate_cover(poem_type: str, poem_id: str, cover_context: str = "", title: str = "") -> str:
    ...
    The per-poem `cover_context` (generated by Mistral) and `title` are
    woven into the FLUX prompt so each poem gets a visually distinct cover,
    rather than every poem of the same type looking identical.
    """
    ...
    # Build a per-poem prompt: lead with the poem-specific visual,
    # anchor with the type's style, then append safety constraints.
    parts = []
    if cover_context:
