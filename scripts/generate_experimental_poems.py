...
def generate_cover(poem_type: str, poem_id: str, cover_context: str = "", title: str = "") -> str:
    """Generate cover image via FLUX (Pollinations primary) with provider fallback chain.

    Chain: Pollinations → Together AI → FluxAPI.ai → Replicate → SVG placeholder.
    Each provider retries 3× on transient errors (429, 5xx, timeout).
    """
    base_style = COVER_PROMPTS.get(poem_type, COVER_PROMPTS["sound"])

    parts = []
    if cover_context:
        parts.append(cover_context.strip().rstrip("."))
    if title:
        parts.append(f"inspired by the poem \"{title}\"")
    parts.append(base_style)
    parts.append(
        "no text, no letters, no words, no hands, no people, no characters, "
        "no faces, children's book illustration style, safe and calming, "
        "dreamy atmosphere"
    )
    prompt = ". ".join(parts)
    ...
    try:
        ...
        from generate_cover_experimental import generate_flux_image
    except Exception as e:
        print(f"  Cover chain import failed: {e}")
        return generate_placeholder_cover(poem_type, poem_id)

    print(f"  Generating FLUX cover (Pollinations primary, fallback chain)...")
    try:
    ...
    if not image_bytes or len(image_bytes) < 1000:
        print(f"  All FLUX providers failed, using placeholder")
        return generate_placeholder_cover(poem_type, poem_id)
...
def generate_poem(age_group: str, poem_type: str, mood: str = "calm",
    ...
    # 5. Generate cover — pass per-poem context so covers aren't identical
    cover_file = generate_cover(poem_type, poem_id, cover_context=cover_context, title=title)
